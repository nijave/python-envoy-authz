...
def test_federation_settings_are_optional(ca_cert_pem, frigate_secret, monkeypatch):
    ...
    assert s.federation() is None


@pytest.mark.parametrize("missing", ["IDP_ISSUER", "SECRET_KEY", "PROVIDERS_FILE"])
def test_federation_requires_all_three_vars(
    ca_cert_pem, frigate_secret, monkeypatch, missing
):
    """Federation cannot be half-enabled: any one var missing turns it off."""
    from envoy_authz.config import Settings

    monkeypatch.setenv("FRIGATE_X_PROXY_SECRET", frigate_secret)
    monkeypatch.setenv("HA_CA_CERTIFICATE", ca_cert_pem)
    monkeypatch.setenv("IDP_ISSUER", "https://idp.example.com")
    monkeypatch.setenv("SECRET_KEY", "sign-the-codes")
    monkeypatch.setenv("PROVIDERS_FILE", "providers.yaml")
    monkeypatch.delenv(missing, raising=False)

    assert Settings(_env_file=None).federation() is None


def test_federation_resolves_when_all_three_are_set(
    ...
    federation = Settings(_env_file=None).federation()
    assert federation is not None
    assert federation.idp_issuer == "https://idp.example.com"
    # Unwrapped from SecretStr so callers do not each reach through it.
    assert federation.secret_key == "sign-the-codes"
    assert federation.providers_file == "providers.yaml"
